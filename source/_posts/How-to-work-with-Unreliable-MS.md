...
- 我下載的是Desktop版，安裝時選擇Mini版本，時區選擇HongKong，最後skip。

（原以爲選中國時區，可以裝完，沒想到還是有147個包，所以還是skip吧。

- 配置橋接網絡靜態IP 192.168.99.96。
...
## 0x03 設置Windows Terminal

- [下載](https://www.microsoft.com/en-us/p/windows-terminal/9n0dx20hk701)，安裝。下面是它的配置文件。（ctrl + ,）
- 由於Terminal默認啓動powershell，因此我們主要修改這一部分，使其自動連接。
            ...
            [
                {
                    ...
                    "name": "Workstations", //  把powershell的名字自定義
                    "commandline": "powershell.exe wsl.exe ssh m4d3bug@192.168.99.96", // 調用wsl來發起鏈接
                    "hidden": false
                },
                {
...
- 為m4d3bug的fish shell和root的bash shell設置proxy

    ```bash
    $ cat /etc/profile.d/bash_proxy.sh                                                                                                               18:20:09#!/bin/bash
    function proxy_off() {
         unset HTTP_PROXY
         unset HTTPS_PROXY
         unset FTP_PROXY
    ...
    $ cat /etc/fish/conf.d/proxy.fish                                                                                                                18:20:11#!/usr/bin/bash
    function proxy_off
         set -e HTTP_PROXY
         set -e HTTPS_PROXY
         set -e FTP_PROXY
    ...
    function proxy_on
         export no_proxy="localhost127.0.0.1,localaddress,.localdomain.com"
         export MY_PROXY_URL="http://192.168.99.99:1081"
         export HTTP_PROXY=$MY_PROXY_URL
         export HTTPS_PROXY=$MY_PROXY_URL
         export FTP_PROXY=$MY_PROXY_URL
